...
- **Workout Display:** User must be able to view workout history for a given exercise and view trends in progress through graphical displays of weight and reps over time (with both weight and reps plotted as two different lines on the same graph), as well as an option to view the increase in calculated 1RM over time. Progress for workouts should be displayed as a list containing each exercise and the percent increase of the calculated 1RM for the exercise over the set period of time determined by the user.
